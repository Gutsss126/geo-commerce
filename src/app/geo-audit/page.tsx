...
function WorkbenchDecisionCard({ decision }: { decision: ReturnType<typeof getGeoWorkbenchDecision> }) {
  const modeStyles = {
    setup: "border-blue-500/30 bg-blue-500/10 text-blue-100",
    config: "border-amber-500/30 bg-amber-500/10 text-amber-100",
    verify: "border-rose-500/30 bg-rose-500/10 text-rose-100",
    task: "border-emerald-500/30 bg-emerald-500/10 text-emerald-100",
    observe: "border-slate-600/70 bg-slate-950/40 text-slate-200",
  };
  const modeLabels = {
    setup: "先建基线",
    ...
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-4xl">
          <span className={`rounded border px-2 py-1 text-xs font-medium ${modeStyles[decision.mode]}`}>
            {modeLabels[decision.mode]}
          </span>
          <p className="mt-3 text-lg font-semibold text-slate-100">{decision.headline}</p>
...
export default async function GeoAuditV2Page() {
  ...
  const workbenchDecision = getGeoWorkbenchDecision({
